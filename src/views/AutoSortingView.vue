  ...
  methods: {
    ...
    createUserPlaylist(title) {
      return new Promise((resolve) => {
        const formattedTitle = `${title[0].toUpperCase()}${title.slice(1)} - Auto-generated by autosort-spotify-liked-songs`;
     ...
     */
    submitTracksToPlaylist(genres, userPlaylist, isForPlaylistAutomaticallyCreated = false) {
      const chunks = [];
      const chunkSize = 100;
      ...
      chunks.forEach((chunk) => {
        ...
        if (chunk.length > 0) {
          ...
          userPlaylist.numberOfItemsAdded = genres.length;
        } else {
